function bs_limo_random_robust(varargin)
...
%                    nboot = nb of resamples
%                    tfce = 1 run tcfe.
% limo_random_robust(7,y1,y2,parameter number,nboot,tfce); %BEHINGER
%                    7 = paired t-test with permutation instead of centering
%                    y1               = data (dim electrodes, frames, subjects)
%                    y2               = data (dim electrodes, frames, subjects)
%                    parameter number = describe which parameters were analysed (e.g. 1 2)
%                    nboot            = nb of resamples
...
if ismember(type, [1 3])
    tfce = varargin{5};
elseif ismember(type, [2 4 5 6 7]); % 7 added by behinger
    tfce = varargin{6};
end
...
switch type
    ...
    case {3}
        ...
        if nboot > 0
            ...
            if bootex == 1;
                ...
                for e = 1:size(array,1)
                    electrode = array(e);
                    if e == 2
                        ti.is=etime(clock,ti.start)/2;
                        ti.est = ti.is*size(array,1);
                    elseif e == 1
                    else
                        fprintf('bootstrapping electrode %g/%g parameter %s \t time remaining %.2f \n',e,size(array,1),num2str(parameter),ti.est-etime(clock,ti.start));
                    end
        ...
        %% BEHINGER
        % All the following is copied from the t-test, and the H0
        % distribution is not done by centering but by resampling.
    case {7}
        data1               = varargin{2};
        data2               = varargin{3}; 
        parameter           = varargin{4}; 
        ...
        % ------------------------------------------------
        % So far exactly the same as t-test paired
        % ------------------------------------------------
        if nboot > 0
...
%                 data2_centered = data2 - repmat(nanmean(data2,3),[1 1 size(data2,3)]);
                ...
                % -------------------------------
                % changes by behinger
                bootTmp = randi([1 2],size(boot_table{1}));
                for el = 1:size(boot_table,2)
                    boot_table{el} = bootTmp; % 1 is keep same order, 2 is permute condition
                end
                % -------------------------------
                save(['H0', filesep, 'boot_table'], 'boot_table')
...
%                     if e == 2
                    ...
                    if e == 1
                        ...
                        ti.dur = 0;
                    elseif e == 2
                        ti.dur = etime(clock,ti.start);
                        ti.start = clock;
                    else
                        ti.dur=(etime(clock,ti.start)+ti.dur)/2;
                        ti.est = ti.dur*length(array)-e;
                        ti.start = clock;
                        fprintf('bootstrapping electrode %g/%g parameter %s \t time remaining %.2f \n',e,size(array,1),num2str(parameter),ti.est-etime(clock,ti.start));
                    end
                    % -------------------------------
                    % changes by behinger
                    tmp = data1(electrode,:,:); Y_tmp(1,:,:,1) = tmp(1,:,find(~isnan(tmp(1,1,:)))); clear tmp;
                    tmp = data2(electrode,:,:); Y_tmp(1,:,:,2) = tmp(1,:,find(~isnan(tmp(1,1,:)))); clear tmp;
                    
                    
                    %                     tmp = data1_centered(electrode,:,:); Y1 = tmp(1,:,find(~isnan(tmp(1,1,:)))); clear tmp
                    %                     tmp = data2_centered(electrode,:,:); Y2 = tmp(1,:,find(~isnan(tmp(1,1,:)))); clear tmp
                    for b=1:nboot
                        bootSamp = boot_table{electrode}(:,b);
                        for kf = 1:length(bootSamp)
                            Y1(1,:,kf) = Y_tmp(1,:,kf,bootSamp(kf));
                            Y2(1,:,kf) = Y_tmp(1,:,kf,abs(bootSamp(kf)-3)); %flip 1 to 2, and 2 to 1
                        end
                        [H0_paired_samples(electrode,:,1,b),diff,se,CI,H0_paired_samples(electrode,:,2,b),tcrit,df]=limo_yuend_ttest(Y1,Y2);
                        % [m,dfe,ci,sd,n,H0_paired_samples(electrode,:,1,b),H0_paired_samples(electrode,:,2,b)]=limo_ttest(1,Y1(1,:,boot_table{electrode}(:,b)),Y2(1,:,boot_table{electrode}(:,b)),0.05);
                    end
                    clear Y1 Y2
                end
...
%             load LIMO; XXX BEHINGER this is unnessecary as LIMO is already loaded...
...
%         save LIMO LIMO; 
        ...
        for i=1:size(tmp_Rep_ANOVA,3)
            ...
            Rep_ANOVA = squeeze(tmp_Rep_ANOVA(:,:,i,:)); % save each factor effect as F/p values
            save([name],'Rep_ANOVA', '-v7.3'); clear Rep_ANOVA;
        end
        
        if type == 3 || type ==4
            for i=1:size(tmp_Rep_ANOVA_Interaction_with_gp,3)
                name = sprintf('Rep_ANOVA_Interaction_gp_Factor_%g',i);
                Rep_ANOVA_Interaction_with_gp = squeeze(tmp_Rep_ANOVA_Interaction_with_gp(:,:,i,:)); % save each interaction effect as F/p values
        ...
        if nboot > 0
            ...
            if bootex == 1;
                ...
                for i=1:size(tmp_boot_H0_Rep_ANOVA,3)
                    ...
                    save(['H0', filesep, name],'H0_Rep_ANOVA', '-v7.3'); clear H0_Rep_ANOVA;
                end
            ...
            if (tfce ~= 0 && size(data1,1) > 1) % check if tfce is on and if more than one electrode
                ...
                for i=1:size(tmp_Rep_ANOVA,3)
                    ...
                    tfce_Rep_ANOVA = limo_tfce(squeeze(Rep_ANOVA(:,:,i,1)),channeighbstructmat);
                    save(tfce_name, 'tfce_Rep_ANOVA'); clear tfce_Rep_ANOVA tmp_Rep_ANOVA
                end
                ...
                for i=1:size(tmp_Rep_ANOVA,3)
                    ...
                    save(tfce_name, 'tfce_H0_Rep_ANOVA'); clear tfce_H0_Rep_ANOVA tmp_boot_H0_Rep_ANOVA
                end
                ...
                if type == 3 || type == 4
                    ...
                    for i=1:size(tmp_Rep_ANOVA_Interaction_with_gp,3)
                        ...
                        save(tfce_name, 'tfce_H0_Rep_ANOVA_Interaction_with_gp'); clear tfce_H0_Rep_ANOVA_Interaction_with_gp tmp_boot_H0_Rep_ANOVA_Interaction_with_gp
                    end
                end
            end
        end
