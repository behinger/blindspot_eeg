function [] = bs_limo_eeg_4_parallel(eIdx)
%% Taken from limo_eeg case{4}
...
if strcmp(LIMO.design.type_of_analysis,'Mass-univariate') && boot_go ==0 %&& ~exist('Betas','file')
...
%             end
            ...
            for e = eIdx%size(array,1)
                ...
                if LIMO.Level == 2
                    ...
                    model = bs_limo_glm1_boot_perm(squeeze(Yr(electrode,:,:))',LIMO,boot_table);
                    warning('using permutation always!!')
                    
                end
                modelTmp{e} = model;
                
                %                                                 H0_Betas(electrode,:,:,:) = model.Betas;
                save(['H0/modelTmp' num2str(eIdx(1))],'modelTmp')
