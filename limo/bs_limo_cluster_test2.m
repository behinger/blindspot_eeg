function [mask, pval, L, NUM, maxclustersum_th] = bs_limo_cluster_test2(ori_f,ori_p,boot_maxclustersum,channeighbstructmat,minnbchan,alpha,boot_distribution)
...
%     if sum(mask(:))>0
        ...
        for CL=1:length(CL_list)
            L(L==CL_list(CL))=CL;
            p(CL)=1-sum( sum(ori_f(L==CL))>=boot_distribution)./length(boot_distribution);
